...
/**
 * An extension for [[sodium.Stream Stream]]s to manipulate finite
 * [[sodium.Stream Stream]] of events.
 */
object FiniteStreamExtension:
  /**
   * A finite [[sodium.Stream Stream]], whose end is signalled to
   * the user as an [[FiniteEvent.EOS EOS (End Of Stream)]]
   * event.
   *
   * Since all [[sodium.Stream Stream]]s are actually unlimited,
   * it isn't possible to prevent a [[sodium.Stream Stream]] from
   ...
   */
  type FiniteStream[A] = sodium.Stream[FiniteEvent[A]]
  ...
  extension [A](self: FiniteStream[A]){
    ...
    /**
     * @param other the specified [[sodium.Stream Stream]].
     * @return a new [[FiniteStream FiniteStream]] that ends each time the
     *         specified [[sodium.Stream Stream]] fires an event.
     * @example {{{
     ...
     * }}}
     */
    private def finiteBy(other: sodium.Stream[?]): FiniteStream[A] =
     ...
     * @param other the specified [[sodium.Stream Stream]].
     * @return a new [[FiniteStream FiniteStream]] that ends when the specified
     *         [[sodium.Stream Stream]] fires an event.
     * @example {{{
     *   s1:                 | E(0) E(1) E(2) E(3) EOS
     *   s2:                 |           a    b
     *   s3:                 |                         0
     ...
     * }}}
     */
    def interruptBy(other: sodium.Stream[?]): FiniteStream[A] =
  ...
  extension [A, Time: Ordering](self: sodium.Stream[(FiniteEvent[A], Time)]){
    /**
