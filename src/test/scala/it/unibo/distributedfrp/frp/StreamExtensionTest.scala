...
class StreamExtensionTest
  ...
  ZipWithTime should
    ...
    "when they are received" in {
    given clock: MockClockScheduler = MockClockScheduler()
    val (sample, s) = load(timedIntSample)
    ...
    zipWithTimeMonitorNow.eventLog shouldEqual Seq(
      0 -> 0.milliseconds,
      1 -> 150.milliseconds,
      2 -> 400.milliseconds,
      3 -> 450.milliseconds,
    ...
    zipWithTimeMonitorShifted.eventLog shouldEqual zipWithTimeMonitorNow.eventLog.map((e,t) => e -> (t + shift))
  }
  ...
  ZipWithDelay should
    ...
    sample.generateEvents()
    zipWithDelayMonitor.eventLog shouldEqual Seq(
      0 -> 0.milliseconds,
      1 -> 150.milliseconds,
      2 -> 250.milliseconds,
      3 -> 50.milliseconds,
  ...
  Drop should "discard the first events generated by a stream" in {
    val (sample, s) = load(stringSample)
    val (_, dropMonitor0) = Stream.monitor(s.drop(0))
    val (_, dropMonitor2) = Stream.monitor(s.drop(2))
    val (_, dropMonitor5) = Stream.monitor(s.drop(5))
    sample.generateEvents()
    dropMonitor0.eventLog shouldEqual Seq("a", "b", "c", "d", "e")
    dropMonitor2.eventLog shouldEqual Seq("c", "d", "e")
    dropMonitor5.eventLog shouldEqual Seq()
  }
