...
class StreamExtensionTest
  ...
  ZipWithTime should
    ...
    "when they were received" in {
    @tailrec
    def strictlyPositiveMonotonic(ys: Seq[Long]): Boolean = ys match
      case y0 :: y1 :: yt => y0 < y1 && strictlyPositiveMonotonic(y1 :: yt)
      case _ => true

    val (sample, s) = load(stringSample)
    val (_, zipWithTimeMonitor) = Stream.monitor(s.zipWithTime)
    sample.generateEvents()

    val events = zipWithTimeMonitor.eventLog.map(_._1)
    val eventTimes = zipWithTimeMonitor.eventLog.map(_._2)
    events shouldEqual Seq("a", "b", "c", "d", "e")
    strictlyPositiveMonotonic(0 +: eventTimes) shouldBe true
  }
