...
class StreamExtensionTest
  ...
  ZipWithIndex should
    ...
    "when they were received" in {
    val (sample, s) = load(stringSample)
    val (_, zipWithIndexMonitorNow) = Stream.monitor(s.zipWithIndex())
    val shift: Int = 10
    val (_, zipWithIndexMonitorShifted) = Stream.monitor(s.zipWithIndex(shift))
    sample.generateEvents()
    zipWithIndexMonitorNow.eventLog shouldEqual Seq(("a", 0), ("b", 1), ("c", 2), ("d", 3), ("e", 4))
    zipWithIndexMonitorShifted.eventLog shouldEqual Seq(("a", 10), ("b", 11), ("c", 12), ("d", 13), ("e", 14))
  }

  ZipWithTime should
    "pair the events of a stream with the instant in the continuous timeline " +
    "when they were received" in {
    @tailrec
    def strictlyPositiveMonotonic[T: Comparator](ys: Seq[T]): Boolean = ys match
      case y0 :: y1 :: yt =>
        summon[Comparator[T]].compare(y0, y1) < 0 &&
        strictlyPositiveMonotonic(y1 :: yt)
      case _ => true

    val (sample, s) = load(stringSample)
    val (_, zipWithTimeMonitorNow) = Stream.monitor(s.zipWithTime())
    val shift: FiniteDuration = 10.seconds
    val (_, zipWithTimeMonitorShifted) = Stream.monitor(s.zipWithTime(shift))
    sample.generateEvents()

    val eventsNow = zipWithTimeMonitorNow.eventLog.map(_._1)
    val eventTimesNow = zipWithTimeMonitorNow.eventLog.map(_._2)
    eventsNow shouldEqual Seq("a", "b", "c", "d", "e")
    strictlyPositiveMonotonic(Duration.Zero +: eventTimesNow) shouldBe true

    val eventsShifted = zipWithTimeMonitorShifted.eventLog.map(_._1)
    val eventTimesShifted = zipWithTimeMonitorShifted.eventLog.map(_._2)
    eventsShifted shouldEqual Seq("a", "b", "c", "d", "e")
    strictlyPositiveMonotonic(shift +: eventTimesShifted) shouldBe true
  }

  ZipWithDelay should
    "pair the events of a stream with the time elapsed in the continuous timeline " +
    "since the previous events" in {
    val (sample, s) = load(stringSample)
    val (_, zipWithDelayMonitor) = Stream.monitor(s.zipWithDelay)
    sample.generateEvents()

    val events = zipWithDelayMonitor.eventLog.map(_._1)
    val eventDelays = zipWithDelayMonitor.eventLog.map(_._2)
    events shouldEqual Seq("a", "b", "c", "d", "e")
    eventDelays forall(_ > Duration.Zero) shouldBe true
  }
  ...
  ThrottleWith should "limit the event production rate (EPR) of a stream to the EPR of a throttler" in {
    val (sample, (s1, s2)) = load(bistreamBitypedLongSample)
    val (_, throttleWithMonitor) = Stream.monitor(s1 throttleWith s2)
    sample.generateEvents()
    throttleWithMonitor.eventLog shouldEqual Seq(
      (2, "a"),
      (3, "c"),
      (4, "e"),
      (5, "f")
  ...
  ThrottleBy should
    ...
    val (sample, (s1, s2)) = load(bistreamBitypedLongSample)
    val (_, throttleByMonitor) = Stream.monitor(s1 throttleBy s2)
    sample.generateEvents()
    throttleByMonitor.eventLog shouldEqual Seq(2, 3, 4, 5)
  }
